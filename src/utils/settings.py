    ...
    def load(self):
        ...
        if not os.path.exists(self.filename):
            if platform == "win32":
                subprocess.call(
                    str(
                        f'copy {os.path.join(os.path.dirname(__file__), "default_settings.json")}',
                        {os.path.join(self.config_dir, self.filename)},
                    ),
                    shell=True,
                )
            elif platform == "linux":
                subprocess.call(
                    str(
                        f'cp {os.path.join(os.path.dirname(__file__), "default_settings.json")}',
                        {os.path.join(self.config_dir, self.filename)},
                    ),
                )
