...
class Program():
    ...
    def __import_modules(self, folder_path: str) -> list[object]:
        file_list = [f[:-3] for f in os.listdir(folder_path) if f.endswith('.py') and f != '__init__.py']
        module_path_name = folder_path.split('/')[-1]
        modules = []
        for module_name in file_list:
            module = importlib.import_module(f'..{module_name}', 
                                             f'program.{module_path_name}.{module_name}')
    ...
    def _setup(self, module):
        if not hasattr(module, 'settings'):
            logger.error('%s does not have attribute module.settings,' +
                         'could not verify needed settings!', module.__module__)
